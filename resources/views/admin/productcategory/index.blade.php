            ...
            $.post("{{url("admin/product/category/ajaxCategoryTree")}}", param, function(data){
               if(data.status == "1"){
                   ...
                   for(var i =0 ; i < list.length; i++){
                       item = list[i];
                       var str = "javascript:itemClick('"+item.id+"', '"+item.title+"', '"+item.img_filename+"','"+item.order_num+"', '"+item.is_leaved+"','"+item.state+"', '"+item.is_main+"','"+item.path+"')";
                       d.add(item.id,item.parent_id,item.title,str);
                   }
