@if(isset($attrInfo))
    ...
    <div class="form-group">
        <label class="col-md-3 control-label" for="name">Require</label>
        <div class="col-md-9">
            <div class="checkbox">
                <label>
                    <input type="checkbox" class="custom-checkbox" id = "is_require" @if($attrInfo['is_require']*1 == 1) checked @endif>&nbsp; IS REQUIRE
                </label>
            </div>
        </div>
    </div>
...
<script>
    $("#editAttrForm").validate({
        ...
        submitHandler: function(form){
            ...
            if(is_variant == 1 && $("select[name='attr_type']").val() != '2'){
                errorMsg("Variant propery is only attribute checkbox  type");
                return;
            }
            var is_require = 0;
            if($("#is_require").prop("checked")){
                is_require = 1;
            }


            $("#editAttrForm input[name='is_variant']").val(is_variant);
            $("#editAttrForm input[name='is_require']").val(is_require);
