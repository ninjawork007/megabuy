                    ...
                    <div class="panel-body">
                        <form class="form-horizontal productInfoForm" id = "productInfoForm" action="javascript:void(0)">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <select class = "form-control" name = "brand_id">
                                        ...
                                        <option  img = "{{correctImgPath($item['log_img'])}}"  value = "{{$item['id']}}" @if($product['brand_id']*1 == $item['id'])  selected @endif>{{$item['title']}}</option>
                                        @endforeach
                                    </select>
                                    <script>
                                        function formatState (state) {
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <select class = "form-control" name = "state">
                                        <option value = "0" @if($product['state']*1 ==0) selected @endif>Draft</option>
                                        <option value = "1" @if($product['state']*1 ==1) selected @endif>Sell</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <select class = "form-control" name = "condition">
                                        <option value = "0" @if($product['condition']*1 ==0) selected @endif>new</option>
                                        <option value = "1" @if($product['condition']*1 ==1) selected @endif>used</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                        ...
                                        @elseif($attr['attr_type']*1 ==3)
                                            @foreach($attr['valList'] as $val)
                                                <label class=" radio-inline">
                                                    <input type="radio" data-product-id = "{{$val['product_id']}}" class = " custom-radio product-attribute"  name = "{{$attr['title']}}" value = "{{$val['id']}}" <?php if($val['checked']*1==1) echo "checked";  ?> >&nbsp;<span>{{$val['val']}}</span></label>
                                            @endforeach
                                        @endif
                                    </div>
                                    @if($attr['attr_type']*1 != 0)
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <button type="button" class="btn btn-responsive btn-default" data-is-require = "{{1-$i}}" onclick = "addAttr(this)" >Customize</button>
                                </div>
                            </div>
                            @endfor
                            <div class="form-group">
    ...
    <script>
        ...
        $("#productInfoForm").validate({
            ...
            submitHandler: function(form){
                ...
                product.state = $("select[name='state']").val();
                ...
                param.product_only_attr_arr = attrVal.attrList
                param.product_only_attr_val_arr = attrVal.attrValList;
        ...
        function getProductVariantArr(){
            ...
            var variant_attr_ids = new Array();
            var variant ;
            ...
            $("#variantTableWrapper tbody tr").each(function(){
                ...
                $(this).find(".val-ids").each(function(){
                    ...
                    variant_attr_ids[variant_attr_ids.length] = val_ids_first;
                });
                variant = new Object();
                variant.price = price;
            ...
            for(var i = 0; i<variant_attr_ids.length; i++){
                var isCheck = false;
                var attr_id
                $(".attr_require").each(function(){
                    attr_id = $(this).attr("data-attr-id");
                    if(variant_attr_ids[i] ==attr_id){
                        isCheck = true;
                    }
                });
        ...
        function getProductAttrVal(){
            ...
            var attrVal ;
            var productAttrArr = new Array();
            var productAttrObj;
            var productAttrValArr = new Array();
            var productAttrValObj;

            var attr_id , val_ids, text_val,valWrapper,attr_type, valEleWrapper , attr_name, attr_val, attr_vals, cur_attr_id;
            $(".attr_require").each(function(){
                ...
                if(attr_id == "-1" || parseInt(attr_id) < -1){
                    if(productAttrArr.length == 0){
                        cur_attr_id = -2;
                    }else{
                        cur_attr_id = productAttrArr[productAttrArr.length-1].id-1;
                    }

                    if(parseInt(attr_id) < -1){
                        cur_attr_id = attr_id;
                    }
                    productAttrObj = new Object();
                    productAttrObj.id =  cur_attr_id;
                    productAttrObj.title = $(this).attr("data-attr-name");
                    productAttrObj.is_require = "1";
                    productAttrObj.attr_type = attr_type;
                    productAttrArr[productAttrArr.length] = productAttrObj;

                    if(attr_type != '0'){
                        switch(attr_type){
                            case "1":
                                valEleWrapper.find("option").each(function(){
                                    productAttrValObj = new Object();
                                    productAttrValObj.attr_id = cur_attr_id;
                                    productAttrValObj.val = $(this).text();
                                    if(productAttrValArr.length ==0){
                                        productAttrValObj.id = -2;
                                    }else{
                                        productAttrValObj.id = productAttrValArr[productAttrValArr.length-1].id-1;
                                    }

                                    productAttrValArr[productAttrValArr.length] = productAttrValObj;
                                    $(this).attr("value", productAttrValObj.id);
                                });
                                break;
                            case "2":
                                $(this).find("input[type='checkbox']").each(function(){
                                    productAttrValObj = new Object();
                                    productAttrValObj.attr_id = cur_attr_id;
                                    productAttrValObj.val = $(this).attr("value1");
                                    if(productAttrValArr.length ==0){
                                        productAttrValObj.id = -2;
                                    }else{
                                        productAttrValObj.id = productAttrValArr[productAttrValArr.length-1].id-1;
                            ...
                            case "1":
                                valEleWrapper.find("option").each(function(){
                                    if($(this).attr("value") == '-1' ){
                                        productAttrValObj = new Object();
                                        productAttrValObj.attr_id = attr_id;
                                        productAttrValObj.val = $(this).text();
                ...
                attrVal = new Object();
                if(attr_id == "-1"){
                    attrVal.attr_id = productAttrArr[productAttrArr.length-1].id;
                    $(this).attr("data-attr-id", productAttrArr[productAttrArr.length-1].id);
                }else{
            ...
            $(".attr_additional").each(function(){
                        ...
                        switch(attr_type){
                            ...
                            case "2":
                                $(this).find("input[type='checkbox']").each(function(){
                                    productAttrValObj = new Object();
                                    productAttrValObj.attr_id = cur_attr_id;
                                    productAttrValObj.val = $(this).attr("name");
                                    if(productAttrValArr.length ==0){
                                        productAttrValObj.id = -2;
                                    }else{
                                        productAttrValObj.id = productAttrValArr[productAttrValArr.length-1].id-1;
                ...
                attrVal.text_val = text_val;
                if(attr_id == "-1"){
                    attrVal.attr_name = $(this).attr("data-attr-name");
                    if(attrVal.attr_name == undefined){
                        return true;
                    }
                }
                if(val_ids != '' || text_val != ''){
            ...
            ret.checked = checked;
            ret.list = attrValArr;
            ret.attrList = productAttrArr;
            ret.attrValList = productAttrValArr;
            return ret;
        ...
        function customizeVariant(){
            ...
            if(curVariantAttrs.length ==0 || curVariantAttrs.length > 3 ){
                errorMsg("Do you can not Variant , Because Attribute is not property, Variant Attribute is require and smaller then 3");
                return;
            }
            var param = new Object();
            param._token = _token;
                ...
                if($(this).attr("data-attr-type") == "2" && $(this).attr("data-attr-is-require") == "1"){
                    ...
                    $(this).find("input[type='checkbox']").each(function(){
                        if($(this).prop("checked")){
                            ...
                            if($(this).attr("value1") == undefined){
                                eleValItem.name = $(this).parent().parent().parent().parent().attr("data-attr-name");
                            }else{
        ...
        function onAddAttrValue(){
            ...
            switch(dataAttrType){
                ...
                case "2":
                    ...
                    $(".attr-value-model-checkbox input[type='checkbox']:eq(0)").attr("value1", value);
                ...
                case "3":
                    ...
                    var name = prevInfoObj.attr("name");
                    $(".attr-value-model-radio input[type='radio']:eq(0)").attr("name", value);
                    $(".attr-value-model-radio input[type='radio']:eq(0)").addClass("custom-radio");
                    $(".attr-value-model-radio span").text(value);
                    html  = $(".attr-value-model-radio").html();
                    parentObj.append(html);
                    $(".attr-value-model-radio input[type='radio']:eq(0)").removeClass("custom-radio");
                    break;
            }
        ...
        function delAttrValue(obj){
            confirmMsg("do you really want to delete this attribute?", function(){
                        ...
                        parentObj.find("input[type='checkbox']").each(function(){
                            if($(this).prop("checked")){
                                if($(this).val()=="-1" ){
                                    ...
                                    var url = "{{url("admin/product/category/ajaxDeleteAttrVal")}}";
        ...
        function onAddAttr(){
            ...
            $(modelClassName + ">.form-group").removeClass("attr_additional");
            $(modelClassName + ">.form-group").removeClass("attr_require");
            ...
            $(modelClassName + " .product-attribute").attr("value1", attr_value);
            if(attr_type == "2" || attr_type == "3"){
                $(modelClassName + " .product-attribute").parent().parent().attr("data-attr-type", attr_type);
                $(modelClassName + " .product-attribute").parent().parent().attr("data-attr-is-require", is_require);
            }else{
            ...
            $(modelClassName + ">.form-group").removeAttr("data-attr-name");
            $(modelClassName + " .product-attribute").attr("name", "");
            $(modelClassName + " .product-attribute").attr("value1", "");
            initInterface();
