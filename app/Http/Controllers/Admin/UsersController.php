...
class UsersController extends JoshController
{
    ...
    {
        $users = User::join('role_users', 'role_users.user_id', '=', 'users.id')->where(function ($query) use ($request) {
            if ($request->has('show_only') && $request->show_only != null && $request->show_only != "" ) {

                $query->where('role_users.role_id', $request->show_only);
            }
        })->get(['users.id', 'users.first_name', 'users.last_name', 'users.email','users.created_at', 'role_users.role_id']);
        ...
        return DataTables::of($users)
            ->editColumn('created_at',function(User $user) {
               ...
               $ret_str = '<span style = "color:black">'.$user['first_name'].'</span>';
               if($user->isTopSeller()){
                   $ret_str = '<span style = "color:blue;">'.$user['first_name'].'</span>';
               }

               return $ret_str;
            })->editColumn('last_name', function(User $user){
                $ret_str = '<span style = "color:black">'.$user['last_name'].'</span>';
                if($user->isTopSeller()){
                    $ret_str = '<span style = "color:blue;">'.$user['last_name'].'</span>';
                }

                return $ret_str;
            })->addColumn('purchased_num',function($user){
                return $user->getPurchasedNum();
            ...
            ->addColumn('sold_num',function($user){
                return $user->getSoldNum();
            })
            ...
            ->rawColumns(['actions','first_name','last_name'])
            ->make(true);
    }
