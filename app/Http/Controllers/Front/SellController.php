...
class SellController extends JoshController
{
    ...
    public function index(Request $request){
	    $id = $request->get("id", "");
	    $brandList = Brand::where(array())->orderBy("id")->get();
    ...
    public function ajaxCategoryTree(Request $request){
    	...
    	if($category_parent_id == "0"){
    		$where = array("is_leaved"=>0, "parent_id"=>$category_parent_id);
    	}else{
    		$where = array( "parent_id"=>$category_parent_id);
    	}
    ...
    public function ajaxSaveSellDetail(Request $request){
        ...
        if(isset($product_detail['id'])){ // update
            if($is_category_changed){ // if category of this product is changed, we will delete all attribute values and save new values
                ...
                for($i = 0; $i < count($attr_id_list); $i++){
                    ...
                    $productAttrValArr['text_val'] = $this->productAttrValModel['text_val'];
                    $info = $this->productAttrValModel->where("product_id", $lastId)
                        ->where("attr_id", $attr_id)->first();
                    if(isset($info['id'])){
                        $this->productAttrValModel->where("product_id", $lastId)
                            ->where("attr_id", $attr_id)
                            ->update($productAttrValArr);
                    }else{
                        $this->productAttrValModel->
                            insert($productAttrValArr);
                    }
        ...
        foreach($users as $user){
            $searchkeys = SearchHistory::where('user_id',$user->id)->get();
            $state = false;
            foreach($searchkeys as $key){
                if($state)
                    continue;
                $value1 = strpos(strtolower($product_detail['title']), strtolower($key->search_key));
                $value2 = strpos(strtolower($product_detail['subtitle']), strtolower($key->search_key));
                $categoryName = ProductCategory::findOrFail($product_detail['category_id'])->title;
                $value3 = strpos(strtolower($categoryName),strtolower($key->search_key)); 
                if($value1 !== false || $value2 !== false || $value3 !== false)
                {
                    $data['title'] = $product_detail['title'];
                    // $data['category'] = $categoryName;
                    // $data['image'] = "assets/images/default-product.png";
                    $data['logo'] = "assets/img/logo_old.png";
                    $searchHistory = new SearchHistory();
                    $data['url'] = $searchHistory->makeSearchUrl($key->id);
                    
                    Mail::to($user->email)
                        ->send(new NewProductMail($data));
                    $state = true;
                }
            }
    ...
    public function ajaxAddBrand(Request $request){
        ...
        $info['log_img'] = $img;
        $info['is_upload_admin'] = "0";
        $info->save();
        $brandList = Brand::where(array())->orderBy("id")->get();
        view()->share("brandList", $brandList);
        return view("front/product/brand_select");
    }
}