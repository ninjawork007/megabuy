...
class ProductController extends JoshController{
    ...
    public function showProduct(Request $request, $id){
        ...
        $temp = explode(",",$commentInfo);
        $returnCondition = '';
        foreach($temp as $row){
            $splits = explode(":",$row);
            if($splits[0] == "Return Condition")
                $returnCondition = $splits[1]; 
        }
        ...
        $where = "eval_mark > 0 AND isProductInOrder($id,id) = 1";
        $model = new Order();
        ...
        return view("/front/product_detail")
            ...
            ->with("returnCondition",$returnCondition)
    ...
    public function ajaxUpdateProductInfo(){
        ...
        if(!isset($info['id'])){
            echo json_encode(array('status'=>'0', "msg"=>'Product info is invalid!'));
            return;
        }
        $num = $info['quantity'];
        if($this->getParam('state','0') == 1){
            $payer = new Payer();
            ...
            $success = url("front/my/seller/activity_selling_drafts");
                
            $urls->setSuccessUrl($success) //success url - the merchant domain page,
            ->setCancelUrl(url("front/my/seller/activity_selling_drafts")); //cancel url - the merchant domain page, to redirect after
            ...
            $info->state = 0;
            $info->save();
            return redirect(url('front/my/seller/activity_selling_product'));
        }
        // if($status){
    ...
    public function payForUpload($amount){
        $payer = new Payer();
        ...
        $success = "http://localhost/backup/public/front/my/seller/activity_selling_drafts";
            
        $urls->setSuccessUrl($success) //success url - the merchant domain page,
        ->setCancelUrl('http://localhost/backup/public/front/checkout/index?immeditaly_type=0'); //cancel url - the merchant domain page, to redirect after
    ...
    public function ajax_order_review_list(){
        ...
        $where = "eval_mark > 0 AND isProductInOrder($product_id,id) = 1";
        $model = new Order();
        $model->orderBy("id", "DESC");
        $model = $model->whereRaw($where);
        $count = $model->count();
