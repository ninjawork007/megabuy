...
class HomeController extends JoshController
{
    ...
    public function index(){
        ...
        $searchCategoryList1 = $categoryModel->where("parent_id", 0)->get();
        foreach($searchCategoryList1 as $key => $rootCategory){
            $id = $rootCategory["id"];
            $leavedCategoryList = $categoryModel
                ->whereRaw("FIND_IN_SET($id, parent_ids)")
                ->where("is_leaved", 1)->where("is_main", "1")->get();
            $searchCategoryList1[$key]["leavedCategoryList"] = $leavedCategoryList;
        }
        view()->share("rootCategory", $searchCategoryList1);
