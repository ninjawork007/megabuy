...
class UserBasket extends Model {
    ...
    static public function getSellerPayments($immeditaly_type =0){
        $user =  Sentinel::getUser();
        $user_id = $user['id'];
        $select = "*, au_user_basket.id basketId";
        $list = DB::table('au_user_basket')
            ...
            ->get();
        $list = convertAttrToArray($list);
        $sellerids = DB::select('select distinct supplier_id from au_user_basket where user_id = ? and immeditaly_type = ?',[$user_id,$immeditaly_type]);
        $sellerids = convertAttrToArray($sellerids);
        $data = array();
        $index = 0;
        foreach($sellerids as $id){
            $data[$index]['amount'] = 0;
            $data[$index]['id'] = $id['supplier_id'];
            $seller = User::findOrFail($id['supplier_id']);
            $data[$index]['merchant_id'] = $seller->merchant_id;
            $data[$index]['merchant_security'] = $seller->merchant_security;
            $index++;
        }
        $temp = array();
        $temp1 = 0;
        foreach($list as $key => $item){
            ...
            foreach($data as $key1 => $row){
                if($item['supplier_id'] == $row['id']){
                    $data[$key1]['amount'] += $list[$key]['price'] * $item['product_count'];
                }
            }
    ...
    static public function createOrder($immeditaly_type =0){
        ...
        foreach($groupList as $item){
            ...
            if(!isset($totalInfo['total_quantity']) || $totalInfo['total_quantity'] == '' ||$totalInfo['total_quantity'] =="0" ){
                continue;
            }
        ...
        // file_put_contents("1.txt",print_r($queries));
        // DB::connection()->enableQueryLog();
        // $queries = DB::getQueryLog();
        return $orderIds;
    }
